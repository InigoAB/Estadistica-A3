...
Intercept es la ordenada en el origen que nos indica el precio en caso de que el resto de variables sean 0. En este caso `r round(lm_2.2.1$coefficients[1],2)` millones es el precio esperado para una casa construida este mismo año, sin que llegue a haber ni un profesor por cada mil habitantes y sin que haya pobreza.

$x_1$ (age) nos indica que el incremento estimado del precio de las casas será de `r round(lm_2.2.1$coefficients[2],2)` por cada incremento de un año desde la fecha de construcción.

La variable $x_2$ (teachers) nos dice que el precio de las casas se estima que incrementará en `r round(lm_2.2.1$coefficients[3],2)` unidades por cada profesor que haya por cada mil habitantes.

La variable $x_3$ (poor_prop) indica que por cada aumento unitario en la proporción de pobreza el precio de las casas disminuirá en `r round(lm_2.2.1$coefficients[4],2)` unidades.
...
Vemos que el coeficiente de determinación ajustado es r`(summary(lm_2.2.1)$adj.r.squared)` por lo que el modelo explica el `r label_percent(accuracy = 0.01)(summary(lm_2.2.1)$adj.r.squared)` de la varianza de las variables. Ha mejorado bastante respecto a los modelos de regresión simple.
...
$x_7$ (airportYES) indica que se estima que el precio de la casa disminuirá en `r round(lm_2.3.1$coefficients[5],2)` millones en caso de que la ciudad tenga aeropuerto.
...
Veo que este nuevo modelo tiene un coeficiente de determinación ajustado es `r (summary(lm_2.3.1)$adj.r.squared)` por lo que el modelo explica el `r label_percent(accuracy = 0.01)(summary(lm_2.3.1)$adj.r.squared)` de la varianza de las variables. Mientras que el anterior explicaba el `r label_percent(accuracy = 0.01)(summary(lm_2.2.4)$adj.r.squared)` de la varianza de las variables.
...
```{r}
predicción <- predict(object = lm_2.2.4,
...
El precio previsto para una vivienda con las características dadas sería de `r round(predicción,2)` millones.
...
Por cada millón que aumenta el precio de la casa la probabilidad de que esté vendida es de entre `r round(exp(confint(lgm_3.1))[2],2)` y `r round(exp(confint(lgm_3.1))[6],2)` veces menor.

Por cada año de antigüedad de la casa la probabilidad de que esté vendida es de entre `r round(exp(confint(lgm_3.1))[3],2)` y `r round(exp(confint(lgm_3.1))[7],2)` veces mayor. Lo que quiere decir que la edad a penas afecta al precio.

Por cada punto de aumento de la proporción de pobreza de la ciudad la probabilidad de que la casa esté vendida es de entre `r round(exp(confint(lgm_3.1))[4],2)` y `r round(exp(confint(lgm_3.1))[8],2)` veces menor.
...
```{r}
odds_ratio_3.3 <- exp(confint(lgm_3.3))[2:5,]
odds_ratio_3.3
```
...
impacto(odds_ratio_3.3)
```

El regresor con mayor impacto es airportYES.
